React.createClass({
  getInitialState: function() {
    ...
    app.connection.emit(
      ...
      function(data) {
        ...
        else state.log = data.log;
        app.connection.on("serverLog", self.serverLog);
      }
    );
    return state;
  },
  ...
  serverLog: function(data) {
    if (data.server != this.props.view.id) return;
    this.setState({
      log: data.clear ? "" : this.state.log + data.log
    });
  },
  componentDidMount: function(prevProps, prevState, rootNode) {
    if (!this.refs.wrapper) return;
    var objLog = this.refs.wrapper;
  ...
  componentWillUnmount: function() {
    app.connection.off("serverLog", this.serverLog);
    window.removeEventListener("scroll", this.scrollStop, false);
  },
  render: function() {
    let log = this.state.log
      .replace(/ /g, "&nbsp;")
      .replace(/\x1b\[((?:\d{1,3};?)+|)m/img, (match, p1) => {
        return "</span><span style='color:" + this.colors[parseInt(p1)] + ";'>";
      });
    log = "<span>" + log.replace(/\n/g, "</span><br/><span>") + "</span>";
